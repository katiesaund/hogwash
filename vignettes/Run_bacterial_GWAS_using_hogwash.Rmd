...
* or use the midpoint rooting method and reorder your phenotype and genotype to the new order.   
```{r}
# Outgroup method
tree_og_rooted <- ape::root.phylo(tree, outgroup = "t4")
tree_og_rooted <- ape::drop.tip(tree, tip = "t4")

growth_phenotype_no_og <- growth_phenotype[row.names(growth_phenotype) %in% tree_og_rooted$tip.label, , drop = FALSE]
growth_phenotype_no_og <- growth_phenotype_no_og[match(row.names(growth_phenotype_no_og), tree_og_rooted$tip.label), , drop = FALSE]

snp_genotype_no_og <- snp_genotype[row.names(snp_genotype) %in% tree_og_rooted$tip.label, , drop = FALSE]
snp_genotype_no_og <- snp_genotype_no_og[match(row.names(snp_genotype_no_og), tree_og_rooted$tip.label), , drop = FALSE]

# Midpoint root method
tree_midpoint <- phytools::midpoint.root(tree)
growth_phenotype_midpoint <- growth_phenotype[row.names(growth_phenotype) %in% tree_midpoint$tip.label, , drop = FALSE]
growth_phenotype_midpoint <- growth_phenotype_midpoint[match(row.names(growth_phenotype_midpoint), tree_midpoint$tip.label), , drop = FALSE]

snp_genotype_midpoint <- snp_genotype[row.names(snp_genotype) %in% tree_midpoint$tip.label, , drop = FALSE]
snp_genotype_midpoint <- snp_genotype_midpoint[match(row.names(snp_genotype_midpoint), tree_midpoint$tip.label), , drop = FALSE]
```
