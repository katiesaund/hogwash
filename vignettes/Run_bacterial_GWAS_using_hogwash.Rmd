...
| sample_3 | 0.05 |  
| sample_4 | 2.70 |    
Ideally, continuous phenotype data would be normally distributed. If your data are not normal, please consider transforming them (e.g. log transform) prior to running hogwash. 

### Genotype
The required structure of the genotype data object is a matrix. The rows correspond to samples and should be ordered to match the tips of the phylogenetic tree. The columns correspond to individual genotypes. The matrix should have both row names and column names. The row names must exactly match the tree’s tip labels. Genotypes can be SNPs (core genome), genes (accessory genome) or other types (indels, pathways, etc...). Genotypes must be coded a binary (0/1).  
Genotype:  
...
### Phylogenetic tree
The phylogenetic tree should be rooted, but if not the tree will be rooted using the midpoint method. The tree must be fully bifurcating. I recommend building your phylogenetic tree with an outgroup, root using the outgroup, and then remove the outgroup prior to running hogwash. 
![phylogenetic tree](https://github.com/katiesaund/hogwash/blob/master/inst/wiki_images/tree_for_wiki.jpg)  
## Optional data
...
The required structure of the grouping genotypes key data object is a matrix. Each row corresponds to one genotype. The first column must have the names of every genotype included in the genotype matrix. The second column must have a name for a group to which the item in the first column belongs. Row names are not required. The column names are used in output plots and therefore must be included. 
...
### Permutation number  
The default value is 10,000. Larger values may take prohibitively long to run.  
...
## All three tests: 
Each test will return a .pdf with at least a manhattan plot of the genetic loci (x-axis) vs. the -ln(p-value) (y-axis). The horizontal red line is the threshold for significance, with significant loci appearing above the line. Only significant loci will be labeled.  
  
![](https://github.com/katiesaund/hogwash/blob/master/inst/wiki_images/2019-06-16_fqR_roary_convergence_test.png)

## Continuous test output plots: for only significant hits
### Phenotype
The ancestral reconstruction of the continuous phenotype. 
![ancestral reconstruction of continuous phenotype](https://github.com/katiesaund/hogwash/blob/master/inst/wiki_images/continuous_phenotype_ancestral_reconstruction.png)

### Genotype
Ancestral reconstruction of the genotype on the tree and the tree with the transition edges identified. 
![genotype transition tree](https://github.com/katiesaund/hogwash/blob/master/inst/wiki_images/Genotype_transitions.png)
...
![histogram](https://github.com/katiesaund/hogwash/blob/master/inst/wiki_images/delta_phenotype_by_edge_type.png)
...
![KS null distribution](https://github.com/katiesaund/hogwash/blob/master/inst/wiki_images/KS_null_distribution.png)
...
![](https://github.com/katiesaund/hogwash/blob/master/inst/wiki_images/ordered_by_tree_edge.png)
...
![heat map summary](https://github.com/katiesaund/hogwash/blob/master/inst/wiki_images/synchronous_grouped_genotype_summary_heatmap.png)

For each significant locus three plots are created: 

1. Phenotype transitions  
![](https://github.com/katiesaund/hogwash/blob/master/inst/wiki_images/synchronous_phenotype_transitio.png)  
2. Genotype transitions  
![](https://github.com/katiesaund/hogwash/blob/master/inst/wiki_images/synchronous_genotype_transition.png)
...
![](https://github.com/katiesaund/hogwash/blob/master/inst/wiki_images/synchronous_null_distribution.png)
...
![](https://github.com/katiesaund/hogwash/blob/master/inst/wiki_images/phenotype_binary_reconstruction.png)
2. Genotype transition   
![](https://github.com/katiesaund/hogwash/blob/master/inst/wiki_images/convergence_genotype_reconstruction.png)
...
![](https://github.com/katiesaund/hogwash/blob/master/inst/wiki_images/convergence_null_distribution.png)
