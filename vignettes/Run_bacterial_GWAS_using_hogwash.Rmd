...
Once a test finishes running it returns data to summarize the results, including a _P_-value for each tested genotype, and plots of the data.
...
The number of edges on the tree where both a genotype mutates (0 &rightarrow; 1) and the phenotype is present occurs is calculated. Then a permutation test is run where the genotype mutations (0 &rightarrow; 1) are randomized on the tree. The number of edges where the permuted genotype mutation (0 &rightarrow; 1) coincides with the phenotype transition is recorded for each permutation, creating a null distribution. An empirical _P_-value is calculated based on the observed number of edges as compared to the null distribution. 
...
* Hogwash reduces the multiple testing burden by testing only those genotype-phenotype pairs for which convergence is detectable; genotypes fewer than 2 transition edges are excluded and genotype-phenotype pairs with fewer than 2 edges where the genotype transition overlaps with phenotype presence are assigned a _P_-value of 1. 
...
The pdf file contains all of the plots while the rda file contains all of the statistics, results, and tables. The figures shown below were generated by running hogwash's implementation of PhyC on a set of simulated data described in the hogwash methods manuscript. 

# Plots

Each test will return a .pdf with a Manhattan plot of the genetic loci (x-axis) vs. the -ln(_P_-value) (y-axis). The horizontal red line is the threshold for significance provided by the user, with significant loci appearing above the line.
  
  
```{r, echo = FALSE, out.width = "400px", }
knitr::include_graphics("hogwash_phyc_phyc_discrete_pheno_BM_tree_1_pheno_1_manhattan.png")
```

Each test will may return a heatmap to show the relationship between the genotype transition edges and the phenotype (either presence, transitions, or |$\Delta$|). The heatmap is only generated if at least two loci were significant. Each column is a genotype. Each row corresponds to an edge on the tree.  
...
- -ln(FDR Corrected _P_-value) (green)
- Locus Signifiance: whether or not the _P_-value is significant after multiple test correction. (Not significant in white, significant in blue)
...
```{r, echo = FALSE, out.width = "400px", }
knitr::include_graphics("hogwash_phyc_phyc_discrete_pheno_BM_tree_1_pheno_1_heatmap.png")
```

Each test returns the phenotype on the tree. PhyC plots the phenotype reconstruction. 

```{r, echo = FALSE, out.width = "400px", }
knitr::include_graphics("hogwash_phyc_phyc_discrete_pheno_BM_tree_1_pheno_1_phenotype.png")
```

## Plots for just the significant hits
Hogwash will return a page in the pdf for each genotype found to be significant. 

The ancestral reconstruction of the continuous phenotype. 

```{r, echo = FALSE, out.width = "400px", }
knitr::include_graphics("hogwash_phyc_phyc_discrete_pheno_BM_tree_1_pheno_1_genotype.png")
```
...
- `$hit_pvals` The FDR corrected _P_-value for each genotype tested.   
- `$sig_pvals` The FDR corrected _P_-value for only genotypes significantly associated with the genotype.  
...
- `$convergence`
  - `$epsilon` The $\varepsilon$ value for each genotype-phenotype pair.  
  - `$pheno_beta` $\beta_{phenotype}$
  - `$geno_beta` $\beta_{genotype}$ for each genotype tested.
  - `$num_hi_conf_edges` Number of high confidence edges for each genotype-phenotype pair.
  - `$intersection` $\beta_{phenotype} \cap  \beta_{phenotype}$ for each genotype-phenotype pair.
...
- `hogwash_continuous$hit_pvals` The FDR corrected _P_-value for each genotype tested.   
- `hogwash_continuous$sig_pvals` The FDR corrected _P_-value for only genotypes significantly associated with the genotype. 
- `hogwash_continuous$non_FDR_hit_pvals`  The _P_-value for each genotype tested prior to FDR correction. 
...
- `$convergence`
  - `$epsilon` The $\varepsilon$ for each genotype-phenotype pair.  
  - `$pheno_beta` $\beta_{phenotype}$ for each genotype-phenotype pair.
  - `$geno_beta` $\beta_{genotype}$ for each genotype tested.
  - `$num_hi_conf_edges` Number of high confidence edges for each genotype-phenotype pair.
  - `$intersection` $\beta_{phenotype} \cap  \beta_{phenotype}$ for each genotype-phenotype pair.
