# 2018-12-14 forked from 2018-11-12 forked from 2018-09-06_phyc_run.R 
# Katie Saund
...
# FORMAT INPUTS ---------------------------------------------------------------#
genotype <- reduce_redundancy(args$genotype, args$tree, args$output_dir, args$output_name) # Remove genotypes that are too rare or too commmon for (1) convergence to be possible and (2) for ancestral reconstruction to work
...
write.table(high_confidence_edges,
            file = paste0(args$output_dir, "/phyc_", args$output_name, "_tree_pheno_length_hi_conf_edges.tsv"), 
            sep = "\t", 
            quote = FALSE, 
            row.names = TRUE, 
            col.names = TRUE)
report_num_high_confidence_trans_edge(geno_trans, all_high_confidence_edges, colnames(genotype), args$output_dir, args$output_name) # added 2018-11-15
...
save_dropped_genotypes(genotype, geno_to_keep, args$output_dir, args$output_name) # 2018-11-13 save genotypes about to be dropped
...
if (args$discrete_or_continuous == "continuous"){
  ...
  save_hits(corrected_pvals_all_transitions$hit_pvals, args$output_dir, args$output_name, "all_transitions_pvals_for_all_hits")
  save_hits(all_transitions_sig_hits,                  args$output_dir, args$output_name, "all_transitions_pvals_for_sig_hits")
  plot_significant_hits(args$tree, args$alpha, args$output_dir, args$output_name, corrected_pvals_all_transitions, phenotype_vector, args$annotation, args$perm, results_all_transitions, pheno_recon_and_conf$node_anc_rec, geno_recon_ordered_by_edges, high_conf_ordered_by_edges, geno_trans, genotype, pheno_recon_edge_mat)
  ...
  save_hits(corrected_pvals_trans$hit_pvals, args$output_dir, args$output_name, "transition_pvals_for_all_hits")
  ...
  if (nrow(corrected_pvals_recon$sig_pvals) > 0){
    ...
    print("No signficiant reconstruction results")
  }
