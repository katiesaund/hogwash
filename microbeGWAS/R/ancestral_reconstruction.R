pick_recon_model <- function(mat, tr, disc_cont, num, recon_method){
  ...
  if (!recon_method %in% acceptable_methods){
...
continuous_ancestral_reconstruction <- function(tr, mat, num, disc_cont, recon_method){
  ...
  reconstruction <- ace(mat[ , num, drop = TRUE],
                        tr,
                        model = "BM",
                        type = disc_cont, method = recon_method,  marginal = FALSE)
...
continuous_get_recon_confidence <- function(recon_vector){
  ...
  tip_and_node_anc_rec_confidence <- rep(1, length(recon_vector)) # Ancestral reconstruction for continuous values only gives back a 95% CI. We can't use any of this information to decide which nodes are low confidence so treat all reconstructed values as high confidence.
