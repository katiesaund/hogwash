...
test_that("run_binary_original() doesn't give any errors when given a discrete phenotype and snps that group into genes" ,{
  ...
  args$test                   <- FALSE
  print(getwd())
  print(paste(getwd(), test_pheno))
  args$phenotype              <- hogwash::read_in_tsv_matrix(test_pheno)
  args$tree                   <- ape::read.tree(test_tree)
  args$tree$node.label[1]     <- 0
  args$tree$node.label        <- as.numeric(args$tree$node.label)
  args$genotype               <- hogwash::read_in_tsv_matrix(test_geno)
  ...
  args$discrete_or_continuous <- hogwash::check_input_format(args$phenotype, args$tree, args$genotype, args$output_name, args$output_dir, args$perm, args$fdr, args$annot)
  args$bootstrap_cutoff       <- as.numeric(test_bootstrap)
  args$group_genotype         <- test_group_genotype
  if (args$group_genotype ) {
    args$gene_snp_lookup      <- hogwash::read_in_tsv_matrix(test_gene_snp_lookup)
  } else {
