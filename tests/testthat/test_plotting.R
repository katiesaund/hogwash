...
# test make_ann_colors
test_that("make_ann_colors works for valid input", {
  # Set up
  test_mat <- matrix(0, ncol = 1, nrow = 1)
  results <- make_ann_colors(test_mat)

  # Test
  expect_equal(names(results), "pheno_presence")
  expect_equal(names(results$pheno_presence), "absent")
  expect_equivalent(results$pheno_presence, "white")
})

test_that("make_ann_colors works for valid input", {
  # Set up
  test_mat <- matrix(c(1, 0), ncol = 1, nrow = 2)
  results <- make_ann_colors(test_mat)

  # Test
  expect_equal(names(results), "pheno_presence")
  expect_equal(names(results$pheno_presence), c("absent", "present"))
  expect_equivalent(results$pheno_presence, c("white", "red"))
})

test_that("make_ann_colors works for valid input", {
  # Set up
  test_mat <- matrix(c(NA, 1), ncol = 1, nrow = 2)
  results <- make_ann_colors(test_mat)

  # Test
  expect_equal(names(results), "pheno_presence")
  expect_equal(names(results$pheno_presence), c("na", "present"))
  expect_equivalent(results$pheno_presence, c("grey", "red"))
})

test_that("make_ann_colors works for valid input", {
  # Set up
  test_mat <- matrix(c(NA, 0), ncol = 1, nrow = 2)
  results <- make_ann_colors(test_mat)

  # Test
  expect_equal(names(results), "pheno_presence")
  expect_equal(names(results$pheno_presence), c("na", "absent"))
  expect_equivalent(results$pheno_presence, c("grey", "white"))
})


test_that("make_ann_colors works for valid input", {
  # Set up
  test_mat <- matrix(c(NA, 0, 1), ncol = 1, nrow = 3)
  results <- make_ann_colors(test_mat)
